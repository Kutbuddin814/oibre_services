...
// Brevo API email sender using HTTPS (port 443) - works on Render free plan
const sendBrevoEmail = async ({ to, subject, html }) => {
  try {
    if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
      console.warn("⚠️ Brevo credentials not configured.");
      return { sent: false, reason: "Brevo credentials not configured" };
    }
...
================================ */
const sendCompletionOtpEmail = async (to, customerName, otp) => {
  const html = `
    <!DOCTYPE html>
    <html>
      <head>
      ...
      <body>
        <div class="container">
          <div class="header">
            <div class="logo">Oibre</div>
            <div class="badge badge-approval">Verification Code</div>
          </div>
          <div class="content">
            <p class="greeting">Hello ${customerName},</p>
            <p class="description">Your service provider has marked the work as complete. Please verify by sharing the code below with the provider:</p>

            <div class="info-box" style="text-align: center; font-size: 24px; font-weight: 700; letter-spacing: 2px; color: #1f2937;">
              ${otp}
            </div>
            ...
            <div class="warning-box">
              <strong>Important:</strong> Do not share this code with anyone except your service provider.
            </div>
  ...
  return sendBrevoEmail({
    to,
    subject: "Service Completion Verification Code - Oibre",
    html
  });
};
...
================================ */
const sendBookingAcceptedEmail = async ({ to, customerName, providerName, serviceName, visitDate, visitTime }) => {
  const html = `
    <!DOCTYPE html>
    <html>
      <head>
      ...
      <body>
        <div class="container">
          <div class="header">
            <div class="logo">Oibre</div>
            <div class="badge badge-approval">Booking Confirmed</div>
          </div>
          <div class="content">
            <p class="greeting">Hello ${customerName},</p>
            <p class="description">Great news! Your service booking has been accepted by the provider.</p>

            <div class="purpose-box approval">
              <div class="purpose-label" style="color: #047857;">Booking Details</div>
              <div class="purpose-text" style="color: #065f46;">Your appointment is confirmed</div>
            </div>

            <div class="info-box">
              <div class="info-item"><span class="info-label">Service:</span> ${serviceName}</div>
              <div class="info-item"><span class="info-label">Provider:</span> ${providerName}</div>
              <div class="info-item"><span class="info-label">Date:</span> ${visitDate}</div>
              <div class="info-item"><span class="info-label">Time:</span> ${visitTime}</div>
            </div>

            <p style="color: #6b7280; font-size: 13px;">Please be ready at the scheduled time. If you need to reschedule, contact the provider through your Oibre account.</p>
  ...
  return sendBrevoEmail({
    to,
    subject: "Your Booking Has Been Accepted - Oibre",
    html
  });
};
